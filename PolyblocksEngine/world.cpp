...
void Brush::render(glm::mat4 viewMatrix, glm::mat4 projectMatrix) {
    ...
    glm::vec3 size = glm::vec3(glm::distance(from.x, to.x) / 2, glm::distance(from.y, to.y) / 2,
                               glm::distance(from.z, to.z) / 2);
    glm::mat4 SizeMatrix = glm::scale(glm::mat4(1.0f), size);
    glm::mat4 ModelMatrix = glm::translate(glm::mat4(1.0f), position) * SizeMatrix;
...
World loadMap(const char *mapName) {
    ...
    for (std::string line; getline(input, line);) {
        ...
        if (line.find("brush") != std::string::npos) {
            ...
            std::vector<double> centerPosition = {(std::stof(pos1[0]) + std::stof(pos2[0])) / 2.0f,
                                                  (std::stof(pos1[1]) + std::stof(pos2[1])) / 2.0f,
                                                  (std::stof(pos1[2]) + std::stof(pos2[2])) / 2.0f};
            // Then we calculate the bounds

            std::vector<double> lowerBound = {
                    centerPosition[0] - std::abs(std::stof(pos1[0]) - std::stof(pos2[0])) / 2.0f,
                    centerPosition[1] - std::abs(std::stof(pos1[1]) - std::stof(pos2[1])) / 2.0f,
                    centerPosition[2] - std::abs(std::stof(pos1[2]) - std::stof(pos2[2])) / 2.0f};
            ...
            index++;
            Brush &br = *new Brush(glm::vec3(std::stof(pos1[0]), std::stof(pos1[1]), std::stof(pos1[2])),
                                   glm::vec3(std::stof(pos2[0]), std::stof(pos2[1]), std::stof(pos2[2])), tex, index);
            br.GenBuffer();
